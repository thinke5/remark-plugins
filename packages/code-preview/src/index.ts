...
export default function remarkCodePreview(
  ...
  return (mdast: Root) => {
    // Add content prefix
    if (contentPrefix) {
      const contentPrefixTree = fromMarkdown(contentPrefix, fromMarkdownOption)

      mdast.children.unshift(...contentPrefixTree.children)
    }
    // Replace code blocks with code previews
